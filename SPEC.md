...
- Timestamps in transcript and pager views include a relative interval after the
  ...
  live for recent entries, switch to minute-level refreshes for older items, and
  use hourly refreshes for very old history.
...
- Repeated configured hallucination phrases are treated as noise even when the audio has energy, preventing looping phrases from surfacing as valid speech.
